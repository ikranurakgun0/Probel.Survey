...
public record DenetimIziDto(long Id, long? KullaniciId, string? KullaniciAdi, string Islem, string? HedefTablo, long? HedefId, DateTime Zaman);
public record HizmetTuruKarsilastirmaDto(string HizmetTuru, int ToplamKatilim, double GenelSkor);
public record KarsilastirmaRaporuDto(List<HizmetTuruKarsilastirmaDto> HizmetTurleri);
public record TopluDavetSonucDto(string HedefMaskeli, bool BasariliMi, string? Token);